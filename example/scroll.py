...
def rgb565_to_rgb255(color_565):
  ...
  # 将 5 位红色缩放到 8 位 (0-255)
  red_8bit = (red_5bit * 255) // 31

  # 将 6 位绿色缩放到 8 位 (0-255)
  green_8bit = (green_6bit * 255) // 63

  # 将 5 位蓝色缩放到 8 位 (0-255)
...
def scroll_info_area(base_image: Image.Image, info_scroll_img: Image.Image, echoview,
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                rgbled = content.get("RGB", 0)
                brightness=content.get("brightness",100)
                scroll_speed = content.get("scroll_speed", 2)
                rgb255_tuple=rgb565_to_rgb255(rgbled)
                ...
                if need_scroll:
                    scroll_thread = threading.Thread(
                        ...
                        args=(img, scroll_img, echoview, scroll_top),
                        kwargs={'scroll_speed': scroll_speed, 'delay': 0.05, 'stop_event': scroll_stop_event}
                    )
                    scroll_thread.start()
                else:
                    rgb565_data = image_to_rgb565(img, echoview.LCD_WIDTH, echoview.LCD_HEIGHT)
