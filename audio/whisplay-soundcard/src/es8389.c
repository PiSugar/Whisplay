...
static bool es8389_volatile_register(struct device *dev, unsigned int reg)
{
	switch (reg) {
	case ES8389_RESET:
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	struct es8389_priv *es8389 = snd_soc_component_get_drvdata(component);
...
static const char *const alc[] = {
	"ALC OFF", "ADCR ALC ON", "ADCL ALC ON", "ADCL & ADCR ALC ON",
};
static const char *const ramprate[] = {
	"0.125db/1 LRCK", "0.125db/4 LRCK", "0.125db/8 LRCK", "0.125db/16 LRCK",
	"0.125db/32 LRCK", "0.125db/64 LRCK", "0.125db/128 LRCK", "0.125db/256 LRCK",
	"0.125db/512 LRCK", "0.125db/1024 LRCK", "0.125db/2048 LRCK", "0.125db/4096 LRCK",
	"0.125db/8192 LRCK", "0.125db/16384 LRCK", "0.125db/32768 LRCK", "0.125db/65536 LRCK",
};
static const char *const winsize[] = {
	"2 LRCK", "4 LRCK", "8 LRCK", "16 LRCK",
	"32 LRCK", "64 LRCK", "128 LRCK", "256 LRCK",
	"512 LRCK", "1024 LRCK", "2048 LRCK", "4096 LRCK",
	"8192 LRCK", "16384 LRCK", "32768 LRCK", "65536 LRCK",
};
...
static const char *const es8389_outl_mux_txt[] = { "Normal", "DAC2 to DAC1" };
static const char *const es8389_outr_mux_txt[] = { "Normal", "DAC1 to DAC2" };
static const char *const es8389_dmic_mux_txt[] = { "AMIC", "DMIC" };
static const char *const es8389_pga1_texts[] = { "DifferentialL", "Line 1P", "Line 2P" };
static const char *const es8389_pga2_texts[] = { "DifferentialR", "Line 2N", "Line 1N" };
static const unsigned int es8389_pga_values[] = { 1, 5, 6 };

static const struct soc_enum es8389_outl_mux_enum =
	SOC_ENUM_SINGLE(ES8389_DAC_MIX, 5, ARRAY_SIZE(es8389_outl_mux_txt), es8389_outl_mux_txt);
...
static const struct soc_enum es8389_outr_mux_enum =
	SOC_ENUM_SINGLE(ES8389_DAC_MIX, 4, ARRAY_SIZE(es8389_outr_mux_txt), es8389_outr_mux_txt);
static const struct snd_kcontrol_new es8389_outr_mux_controls =
	SOC_DAPM_ENUM("OUTR MUX", es8389_outr_mux_enum);

static SOC_ENUM_SINGLE_DECL(es8389_dmic_mux_enum, ES8389_DMIC_EN, 6, es8389_dmic_mux_txt);

static const struct soc_enum es8389_pgal_enum =
	SOC_VALUE_ENUM_SINGLE(ES8389_MIC1_GAIN, 4, 7,
		ARRAY_SIZE(es8389_pga1_texts), es8389_pga1_texts, es8389_pga_values);
static const struct soc_enum es8389_pgar_enum =
	SOC_VALUE_ENUM_SINGLE(ES8389_MIC2_GAIN, 4, 7,
		ARRAY_SIZE(es8389_pga2_texts), es8389_pga2_texts, es8389_pga_values);
...
static const struct snd_kcontrol_new es8389_snd_controls[] = {
	...
	SOC_SINGLE_TLV("ALC Capture Target Level", ES8389_ALC_TARGET, 0, 0x1f, 0, alc_target_tlv),
	SOC_SINGLE_TLV("ALC Capture Max Gain", ES8389_ALC_GAIN, 0, 0x1f, 0, alc_max_level),
...
static const struct snd_soc_dapm_widget es8389_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIXER("IF DACL Mixer", SND_SOC_NOPM, 0, 0,
		&es8389_left_mixer_controls[0], ARRAY_SIZE(es8389_left_mixer_controls)),
	SND_SOC_DAPM_MIXER("IF DACR Mixer", SND_SOC_NOPM, 0, 0,
		&es8389_right_mixer_controls[0], ARRAY_SIZE(es8389_right_mixer_controls)),
	SND_SOC_DAPM_MIXER("IF ADCDACL Mixer", SND_SOC_NOPM, 0, 0,
		&es8389_leftadc_mixer_controls[0], ARRAY_SIZE(es8389_leftadc_mixer_controls)),
	SND_SOC_DAPM_MIXER("IF ADCDACR Mixer", SND_SOC_NOPM, 0, 0,
		&es8389_rightadc_mixer_controls[0], ARRAY_SIZE(es8389_rightadc_mixer_controls)),
	SND_SOC_DAPM_MIXER("ADC Mixer", SND_SOC_NOPM, 0, 0,
		&es8389_adc_mixer_controls[0], ARRAY_SIZE(es8389_adc_mixer_controls)),
...
static const struct snd_soc_dapm_route es8389_dapm_routes[] = {
	...
	{"OUTL MUX", "DAC2 to DAC1", "IF ADCDACR Mixer"},
	{"OUTR MUX", "Normal", "IF ADCDACR Mixer"},
	{"OUTR MUX", "DAC1 to DAC2", "IF ADCDACL Mixer"},
	{"HPOL", NULL, "OUTL MUX"},
	{"HPOR", NULL, "OUTR MUX"},
};

struct _coeff_div {
	u16 fs; u32 mclk; u32 rate;
	u8 Reg0x04, Reg0x05, Reg0x06, Reg0x07, Reg0x08, Reg0x09, Reg0x0A;
	u8 Reg0x0F, Reg0x11, Reg0x21, Reg0x22, Reg0x26, Reg0x30;
	u8 Reg0x41, Reg0x42, Reg0x43, Reg0xF0, Reg0xF1, Reg0x16, Reg0x18, Reg0x19;
	u8 dvdd_vol, dmic_sel;
};

/* MCLK clock divider coefficient table (full) */
static const struct _coeff_div coeff_div[] = {
	...
	/* 24k, 32k, 44.1k, 48k, 88.2k, 96k, 192k - selected entries for 24.576MHz and 24MHz MCLK */
	...
	{128, 12288000, 96000, 0x01,0x41,0x00,0xD0,0x10,0xD1,0x80, 0x40,0x00,0x9F,0x7F,0xBF,0xC0, 0x7F,0x7F,0x80,0x12,0xC0,0x35,0x91,0x28, 2,2},
...
static int es8389_set_dai_sysclk(struct snd_soc_dai *dai, int clk_id,
				...
				struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (es8389->have_supplies) {
		ret = regulator_get_voltage(
				es8389->core_supply[ES8389_SUPPLY_VD].consumer);
		switch (ret) {
	...
	if (coeff >= 0) {
		...
		regmap_update_bits(es8389->regmap, ES8389_OSC_CLK, 0xC0, coeff_div[coeff].Reg0x0F);
		...
		/*
		 * ES8389_OSR_VOL is exposed as the ADC OSR Volume mixer and is
		 * part of Whisplay's calibrated capture LUT.  Do not overwrite it
		 * from the clock coefficient table on every capture open, or the
		 * user-facing Capture Gain loses its high-end range.
		 */
		regmap_update_bits(es8389->regmap, ES8389_SYSTEM30, 0xC0, coeff_div[coeff].Reg0x30);
		...
		regmap_update_bits(es8389->regmap, ES8389_DAC_MISC, 0x81, coeff_div[coeff].Reg0x43);
		regmap_update_bits(es8389->regmap, ES8389_CHIP_MISC, 0x72, coeff_div[coeff].Reg0xF0);
		...
		dev_warn(component->dev,
			"Clock coefficients not found: mclk=%u rate=%u vddd=%u dmic=%u",
			es8389->sysclk, params_rate(params), es8389->vddd, dmic_enable);
		return coeff;
	}
...
static int es8389_mute(struct snd_soc_dai *dai, int mute, int direction)
{
	...
	if (mute) {
		...
		if (direction == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			regmap_update_bits(es8389->regmap, ES8389_DAC_FORMAT_MUTE, 0x03, 0x00);
		} else {
...
struct snd_soc_dai_driver es8389_dai = {
	.name = "ES8389 HiFi",
	.playback = {
...
static int es8389_component_probe(struct snd_soc_component *component)
{
	struct es8389_priv *es8389;
	int ret, i;

	es8389 = devm_kzalloc(component->dev, sizeof(*es8389), GFP_KERNEL);
	if (!es8389)
		return -ENOMEM;
	...
	if (ret != 0) {
		dev_dbg(component->dev, "mclk-src not specified, defaulting to MCLK pin\n");
		es8389->mclk_src = ES8389_MCLK_SOURCE;
	}

	for (i = 0; i < ARRAY_SIZE(es8389_core_supplies); i++)
		es8389->core_supply[i].supply = es8389_core_supplies[i];
	ret = devm_regulator_bulk_get(component->dev, ARRAY_SIZE(es8389_core_supplies),
				      es8389->core_supply);
	if (ret) {
		dev_warn(component->dev,
			 "Core supplies not available (%d), assuming 3.3V\n", ret);
		es8389->have_supplies = false;
		es8389->vddd = ES8389_3V3;
	} else {
		es8389->have_supplies = true;
	}
	...
	es8389_chip_init(component);
	es8389_set_bias_level(component, SND_SOC_BIAS_STANDBY);

	return 0;
}

static void es8389_component_remove(struct snd_soc_component *component)
{
	struct es8389_priv *es8389 = snd_soc_component_get_drvdata(component);
...
const struct regmap_config es8389_regmap_config = {
	.reg_bits = 8,
	.val_bits = 8,
...
EXPORT_SYMBOL_GPL(es8389_regmap_config);

struct snd_soc_component_driver es8389_component_driver = {
	.probe = es8389_component_probe,
	.remove = es8389_component_remove,
...
EXPORT_SYMBOL_GPL(es8389_component_driver);

/*
 * Apply Whisplay-calibrated default gains.
 *
 * Tuned together with the WM8960 path so both boards deliver roughly
 * the same SPL at the headphone jack for a -3 dBFS digital tone, and
 * the same capture envelope for the same acoustic input on PGA1/PGA2.
 *
 * TLV math for DAC/ADC/OSR volumes:
 *   step 0.5 dB, min -95.5 dB, max at raw 191 (=0xBF) = 0 dB.
 *   so raw N -> (N * 0.5 - 95.5) dB.
 *
 * Values may be overridden at any time by amixer or alsactl restore.
 */
void es8389_apply_default_gains(struct snd_soc_component *component)
{
	struct es8389_priv *es8389 = snd_soc_component_get_drvdata(component);

	if (!es8389 || !es8389->regmap)
		return;

	/* Digital playback path:
	 * DACL/DACR = 0x7E (126) -> -32.5 dBFS.
	 * Observer-calibrated against WM8960 HP=15 playback output.
	 */
	regmap_write(es8389->regmap, ES8389_DACL_VOL, 0x7E);
	regmap_write(es8389->regmap, ES8389_DACR_VOL, 0x7E);

	/* Capture at unified gain 50 (see whisplay_es8389_pga/adc/osr_lut):
	 * PGA analog + digital ADC/OSR split (0 dB digital each).
	 */
	regmap_write(es8389->regmap, ES8389_ADCL_VOL, 0xBF);
	regmap_write(es8389->regmap, ES8389_ADCR_VOL, 0xBF);
	regmap_write(es8389->regmap, ES8389_OSR_VOL, 0xBF);
	regmap_update_bits(es8389->regmap, ES8389_ADC_MUTE, 0xC0, 0xC0);

	regmap_update_bits(es8389->regmap, ES8389_MIC1_GAIN, 0x0F, 0x0B);
	regmap_update_bits(es8389->regmap, ES8389_MIC2_GAIN, 0x0F, 0x0B);

	/* Mixer volume: -32 dB for ADC->DAC monitor path. */
	regmap_write(es8389->regmap, ES8389_MIX_VOL, 0x40);

	dev_info(component->dev,
		 "ES8389 Whisplay defaults: DAC=-32.5dB, cap PGA=11 ADC/OSR=0dB\n");
}
EXPORT_SYMBOL_GPL(es8389_apply_default_gains);

int es8389_chip_init(struct snd_soc_component *component)
{
	struct es8389_priv *es8389 = snd_soc_component_get_drvdata(component);
	unsigned int reg;
	...
	regmap_update_bits(es8389->regmap, ES8389_MIC1_GAIN, ES8389_MIC_SEL_MASK, ES8389_MIC_DEFAULT);
	regmap_update_bits(es8389->regmap, ES8389_MIC2_GAIN, ES8389_MIC_SEL_MASK, ES8389_MIC_DEFAULT);
...
EXPORT_SYMBOL_GPL(es8389_chip_init);
